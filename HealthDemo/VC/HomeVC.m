...
- (void)saveRecord{
    ...
    if (saveResult) {
        ...
        if (_syncSwitch.isOn) {
            NSLog(@"开始同步");
            [HealthKitTool saveRecord:record withCompletion:^(BOOL success, NSError * _Nullable error) {
                //结果回调函数
...
- (void)toHistoryTableVC{
    ...
    UINavigationController *naviC = [[UINavigationController alloc] initWithRootViewController:historyTVC];
...
//        make.top.equalTo(@(100));
    ...
    [thunder mas_makeConstraints:^(MASConstraintMaker *make) {
            make.height.width.equalTo(@(16));
            make.right.equalTo(sw);
            make.bottom.equalTo(sw.mas_top).offset(8);
    }];
...
//            make.height.equalTo(@(64));
    //        make.width.equalTo(@(64));
            make.centerY.equalTo(stack);
        make.right.equalTo(sw.mas_left);
        make.left.equalTo(stack);
    }];
    [stack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@(64));
        make.top.equalTo(input.mas_bottom);
            make.left.equalTo(input);
            make.right.equalTo(input);
    }];
    ...
    [save mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(input);
        make.right.equalTo(input);
        make.top.equalTo(stack.mas_bottom).offset(16);
    }];
    [history mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@(32));
        make.left.equalTo(input);
        make.right.equalTo(input);
        make.top.equalTo(save.mas_bottom).offset(16);
    }];
}
